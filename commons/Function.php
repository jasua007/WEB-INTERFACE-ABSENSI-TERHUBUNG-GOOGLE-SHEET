...
if(!isset($_SESSION['logged_in'])){
    ...
    global $db;
    $db = mysqli_connect('localhost','root','','sman1_pematangsiantar');
    if(!$db){
        die("Database Connect Problem");
    }
// Check connection
if (mysqli_connect_errno()){
	echo "Database connect problem: " . mysqli_connect_error();
}
    ...
    function TambahAbsensi($kode_guru, $id_jadwal, $kelas){
        global $db;
        $query = "INSERT INTO absensi VALUES('','$kode_guru','$id_jadwal','$kelas')";
        $result = ExecuteQuery($query);    
    ...
    function TambahAkun($username,$password,$role){
        $query = "INSERT INTO akun VALUES('$username','$password','$role')";
        $result = ExecuteQuery($query);
    }
    function UpdateAkun($username,$new_username,$new_password){
        $query = "UPDATE akun SET username = '$new_username', PASSWORD = $new_password WHERE username = '$username'";
        $result = ExecuteQuery($query);
    }
    ...
    function getAllSiswa(){
        $query = "SELECT * FROM siswa ORDER BY kelas,nama ASC";
        $result = ExecuteQuery($query);
        $AllSiswa = [];
    ...
    function getAllAccount(){
        $query = "SELECT * FROM akun WHERE role = '2'";
        $result = ExecuteQuery($query);
    ...
    function cekKelas($kelas){
        $query = "SELECT * FROM kelas WHERE nama_kelas = '$kelas'";
        $result =ExecuteQuery($query);
        if(mysqli_num_rows($result) > 0){
            return 1;
        }
    ...
    function TambahJadwal($DataBaru){
        $id_jadwal = $DataBaru['id_jadwal'];
        $jam_mulai = $DataBaru['jam_mulai'];
        $jam_berakhir = $DataBaru['jam_berakhir'];
        $kode_guru = $DataBaru['kode_guru'];
        $kelas = $DataBaru['kelas'];

        $query = "INSERT INTO jadwal VALUES('$id_jadwal','$jam_mulai','$jam_berakhir','$kode_guru','$kelas')";
        $result = ExecuteQuery($query);
    }
    function UpdateJadwal($id_jadwal_lama,$DataBaru){
        $id_jadwal = $DataBaru['id_jadwal'];
        $jam_mulai = $DataBaru['jam_mulai'];
        $jam_berakhir = $DataBaru['jam_berakhir'];
        $kode_guru = $DataBaru['kode_guru'];
        ...
        $query = "UPDATE jadwal SET id_jadwal = '$id_jadwal',jam_mulai = '$jam_mulai',jam_berakhir = '$jam_berakhir',
                 kode_guru  = '$kode_guru',kelas = '$kelas' WHERE id_jadwal = '$id_jadwal_lama'";
        $result = ExecuteQuery($query);
    }
    ...
    function getAllJadwalGuru($kode_guru){
        $query = "SELECT * FROM jadwal WHERE kode_guru = '$kode_guru' ORDER BY kelas";
        $result = ExecuteQuery($query);
    ...
    function UpdateAbsensi($NISN,$id_absen,$kehadiran){
         echo "kehadiran = ". $kehadiran."<br>".'ID Absen : '.$id_absen."<br>";
